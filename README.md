...
идейный аналог Ericsson Dynamic Activation (EDA), но под нейтральным именем
и полностью открытой реализацией. Разворачивается как обычный сетевой элемент
на OpenStack (Nova instance / Docker), управляется через REST API и CLI.

Архитектура сверена с официальным описанием Ericsson EDA (микросервисы
eric-act-*, блоки Inbound Interfaces / OAM / Activation Core / Activation
Support / Security & Connectivity / Data) — см. `docs/ARCHITECTURE.md` для
...
(формат аналогичен параметрическому файлу Ericsson EDA: `type`, `path`,
...
docs/             # архитектурные заметки, таблица соответствий с Ericsson EDA, каталог параметров
